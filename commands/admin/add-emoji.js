...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    if (!message.member.hasPermission(`MANAGE_EMOJIS`)) {
      return message.channel.send(`❌ You Don't Have Permission Manage Emojis`)
    }

    if (!message.guild.me.hasPermission("MANAGE_EMOJIS")) {
      return message.channel.send(`❌ I dont have perms to upload emoji, give me Permissions right now!`)
    }
    ...
    if (customemoji.id) {
      ...
      message.channel.send(
        `You Can Use Normal Emoji anywhere you want in discord!`
      );
    }
  }
};
