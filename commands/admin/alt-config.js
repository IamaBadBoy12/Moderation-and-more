  ...
  config: {
	...
	run: async (bot, message, args) => {
		if (!message.member.hasPermission('MANAGE_GUILD'))
			return message.channel.send(
				`❌ You must have the \`MANAGE_GUILD\` permission to run the command.`
			);
		...
		if (!option)
			return message.channel.send(
				`❌ | The option argument must be one of:\n **${options.join(', ')}**`
			);
		function check(opt) {
			return options.includes(opt);
		}
		if (!check(option.toLowerCase())) {
			return message.channel.send('❌  | **The provided option is invalid**');
		}
    ...
    if(day > 100) return message.channel.send("❌ Please donot exceed the length of 100 days");
			...
			case 'logs':
				...
				if (!channel)
					return message.channel.send('❌  | **Specify the channel**');
